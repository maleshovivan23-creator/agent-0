...
| Складывает готовое в очередь `inbox`, собирает досье по задаче | Заводит кошелёк/счёт для выплат |
| Следит за взятыми задачами: соперник, закрытие, выплата | Отправляет заявку и делает работу |
...
## Что робот проверяет сам (дозор)

```bash
python -m agent.main дозор            # проблемы и что с ними делать
python -m agent.main дозор --json     # для мониторинга: код возврата 1 при проблеме
```

| Проверка | Порог (настраивается) | Что означает |
|---|---|---|
| Робот молчит | больше двух интервалов | процесс упал или машина уснула |
| Канал без находок | `WATCHDOG_QUIET_RUNS` проходов (6) | нужны новые запросы или ключ |
| Канал с ошибками | половина проходов из последних N | сеть или блокировка площадки |
| Лимит GitHub | остаток 0 | проход будет пустым без `GITHUB_TOKEN` |
| Очередь ждёт человека | `WATCHDOG_NAG_HOURS` часов (12) | узкое место — не робот, а отправка |

Проблемы того же вида не повторяются чаще, чем раз в `AUTOPILOT_NUDGE_HOURS` (6 ч):
иначе уведомления начнут раздражать и их выключат. Каналы, которые молчат по делу
(нет ключа площадки, нет `data/scope.yaml`), в проблемы не попадают.

Мониторинг по расписанию, если нет systemd:

```bash
# в crontab: раз в 30 минут проверить дозор и при проблеме написать в Telegram
*/30 * * * * cd /opt/agent-0 && .venv/bin/python -m agent.main дозор --json >/dev/null || \
  .venv/bin/python -c "from agent.telegram import TelegramNotifier as T; T().send('AGENT-0: дозор нашёл проблему, смотри journalctl')"
```
